...
class Project(NamedItem):
    ...
    def run_sampled_sims(self, parset, progset=None, progset_instructions=None, result_names=None, n_samples: int = 1, rand_seed: int = None, parallel=False, max_attempts=None, num_workers=None, custom_fct=None) -> list:
        ...
        if parallel:
            ...
            with Quiet():
                for i,rng in enumerate(tqdm.tqdm(model_rngs)):
                    if isinstance(progset,list):
                        ps = progset[i]
                    else:
                        ps = progset
...
def _run_sampled_sim(proj, parset, progset, progset_instructions: list, result_names: list, rng_sampler=None, max_attempts: int = None):
    """
    ...
    while attempts < max_attempts:
        try:
            ...
            if progset:
                ...
                results = [proj.run_sim(parset=sampled_parset, progset=sampled_progset, progset_instructions=x, result_name=y, rng=rng_attempt) for x, y in zip(progset_instructions, result_names)]
            else:
                results = [proj.run_sim(parset=sampled_parset, result_name=y, rng=rng_attempt) for y in result_names]
            return results
