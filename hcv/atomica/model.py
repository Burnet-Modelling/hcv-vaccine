...
class Compartment(Variable):
    ...
    def __init__(self, pop, name, rng_sampler=None):
...
class JunctionCompartment(Compartment):
    def __init__(self, pop, name: str, duration_group: str = None, rng_sampler=None):
        """
        ...
        """
        super().__init__(pop, name, rng_sampler=rng_sampler)
...
class TimedCompartment(Compartment):
    def __init__(self, pop, name: str, parameter: ParsetParameter, rng_sampler=None):
        """
        ...
        Compartment.__init__(self, pop=pop, name=name, rng_sampler=rng_sampler)
...
class Population:
    ...
    def __init__(self, framework, name: str, label: str, progset: ProgramSet, pop_type: str, rng_sampler=None):
        """
        ...
        for comp_name in list(comps.index):
            if comps.at[comp_name, "population type"] == self.type:
                if comp_name in residual_junctions:
                    self.comps.append(ResidualJunctionCompartment(pop=self, name=comp_name, rng_sampler=self.rng_sampler, duration_group=comps.at[comp_name, "duration group"]))
                elif comps.at[comp_name, "is junction"] == "y":
                    self.comps.append(JunctionCompartment(pop=self, name=comp_name, rng_sampler=self.rng_sampler, duration_group=comps.at[comp_name, "duration group"]))
                elif comps.at[comp_name, "duration group"]:
                    self.comps.append(TimedCompartment(pop=self, name=comp_name, rng_sampler=self.rng_sampler, parameter=self.par_lookup[comps.at[comp_name, "duration group"]]))
                elif comps.at[comp_name, "is source"] == "y":
                    self.comps.append(SourceCompartment(pop=self, name=comp_name, rng_sampler=self.rng_sampler))
                elif comps.at[comp_name, "is sink"] == "y":
                    self.comps.append(SinkCompartment(pop=self, name=comp_name, rng_sampler=self.rng_sampler))
                else:
                    self.comps.append(Compartment(pop=self, name=comp_name, rng_sampler=self.rng_sampler))
...
class Model:
    ...
    def build(self, parset):
        ...
        for k, (pop_name, pop_label, pop_type) in enumerate(zip(parset.pop_names, parset.pop_labels, parset.pop_types)):
            self.pops.append(Population(framework=self.framework, name=pop_name, label=pop_label, progset=self.progset, pop_type=pop_type, rng_sampler=self.rng_sampler))
